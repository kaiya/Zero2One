...
@interface KYInternalAudioRecorderController : UIViewController <AVAudioRecorderDelegate,AVAudioPlayerDelegate,UIActionSheetDelegate>
{
    //Recording...
    AVAudioRecorder *_audioRecorder;
    KYSiriWaveformView *musicFlowView;
...
@property(nonatomic, weak) id<KYAudioRecorderControllerDelegate> delegate;

@property(nonatomic, assign) BOOL shouldShowRemainingTime;
@end


@implementation KYAudioRecorderController
{
    KYInternalAudioRecorderController *_internalController;
}
...
-(void)viewDidLoad
{
    ...
    _internalController = [[KYInternalAudioRecorderController alloc] init];
...
-(void)setDelegate:(id<KYAudioRecorderControllerDelegate,UINavigationControllerDelegate>)delegate
{
    _delegate = delegate;
...
@implementation KYInternalAudioRecorderController
...
-(void)loadView
{
    ...
    musicFlowView = [[KYSiriWaveformView alloc] initWithFrame:view.bounds];
...
//    _recordingFilePath = [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.m4a",fileName]];
    _recordingFilePath = [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.pcm",fileName]];
...
//        _recordButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"audio_record"] style:UIBarButtonItemStylePlain target:self action:@selector(recordingButtonAction:)];
         _recordButton = [[UIBarButtonItem alloc]initWithTitle:@"Record" style:UIBarButtonItemStylePlain target:self action:@selector(recordingButtonAction:)];
...
-(void)cancelAction:(UIBarButtonItem*)item
{
    ...
    {
        KYAudioRecorderController *controller = (KYAudioRecorderController*)[self navigationController];
